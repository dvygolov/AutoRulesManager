...
const SOURCE = path.join(ROOT, "autorules-manager.js");
const LOADER_SOURCE = path.join(ROOT, "autorules-loader.js");
const LANDING_SCREENSHOT = path.join(ROOT, "target-current.png");
const OUT_ROOT = path.join(ROOT, "dist", "autorules");
const CHUNK_SIZE = 350000;
const APP_MARK_FILE = "assets/autorules-mark.svg";
...
function buildAppMarkSvg() {
  return [
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 96 96" role="img" aria-label="AutoRulesManager mark">',
    '  <defs>',
    '    <linearGradient id="autorules-gold" x1="0%" x2="100%" y1="0%" y2="100%">',
    '      <stop offset="0%" stop-color="#ffe16a"/>',
    '      <stop offset="55%" stop-color="#ffd000"/>',
    '      <stop offset="100%" stop-color="#ffab00"/>',
    '    </linearGradient>',
    '  </defs>',
    '  <rect x="4" y="4" width="88" height="88" rx="22" fill="#151515" stroke="url(#autorules-gold)" stroke-width="6"/>',
    '  <path d="M28 30h32c4.4 0 8 3.6 8 8v28c0 4.4-3.6 8-8 8H28c-4.4 0-8-3.6-8-8V38c0-4.4 3.6-8 8-8Z" fill="#222" stroke="#fff2bd" stroke-width="4"/>',
    '  <path d="M33 44h25M33 56h19" fill="none" stroke="url(#autorules-gold)" stroke-width="5" stroke-linecap="round"/>',
    '  <path d="m61 23-9 20h10l-7 19 20-28H64l8-11Z" fill="url(#autorules-gold)"/>',
    '  <path d="m26 46 5 5 9-12M26 59l5 5 9-12" fill="none" stroke="#ffd000" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>',
    '</svg>',
  ].join("\n");
}
...
function detectBuild(source) {
  ...
  if (!match) {
    throw new Error("Cannot detect AutoRulesManager VERSION in source file.");
  }
  return match[1];
}
...
function buildLandingHtml({ appName, displayName = appName, build, bookmarklet, manifestUrl, screenshotUrl, iconUrl }) {
  ...
  return [
    ...
    `  <title>${escapeHtml(title)}</title>`,
    '  <meta name="robots" content="noindex,nofollow" />',
    `  <meta name="description" content="${escapeHtml(displayName)} bookmarklet loader for Facebook Ads Manager automated rules export, import, status, and currency-safe migration workflows." />`,
    ...
    "          <h1>Auto rules management without leaving Ads Manager.</h1>",
    "          <p class=\"lead\">AutoRulesManager is a browser-side tool for exporting, importing, deleting, enabling, and disabling Facebook automated rules across ad accounts, with batch API calls and currency-aware rule thresholds.</p>",
    "          <div class=\"install\">",
    `            <a class="bookmarklet" id="bookmarkletLink" href="${escapeHtml(bookmarklet)}">AutoRules</a>`,
    ...
    "        </div>",
    "        <figure class=\"shot\">",
    `          <img src="${escapeHtml(screenshotUrl)}" alt="AutoRulesManager running inside Facebook Ads Manager" />`,
    "        </figure>",
    "      </div>",
    "    </section>",
    "    <section class=\"section cards\" id=\"features\">",
    "      <div class=\"card\"><strong>Export</strong><p>Save automated rules from a selected ad account to a reusable JSON package with rule metadata.</p></div>",
    "      <div class=\"card\"><strong>Import</strong><p>Upload a rules JSON into one or many accounts through batched Ads Manager Graph requests.</p></div>",
    "      <div class=\"card\"><strong>Clean up</strong><p>Delete existing rules before import when a cabinet needs a clean rule set instead of duplicates.</p></div>",
    "      <div class=\"card\"><strong>On / Off</strong><p>Enable or disable all automated rules in selected accounts from one compact panel.</p></div>",
    "      <div class=\"card\"><strong>Currency-aware</strong><p>Converts prefixed money fields like ad.today_spent and campaign.lifetime_cost_per_lead_fb between account currencies.</p></div>",
    "      <div class=\"card\"><strong>Batch-safe</strong><p>Processes rules and accounts in throttled batches, keeping per-account logs visible in the UI.</p></div>",
    ...
    "      <div class=\"step\"><b>1. Install</b>Drag the AutoRules button to your bookmarks bar, or use Copy to clipboard and paste the loader into a bookmark URL.</div>",
    ...
    "      <div class=\"step\"><b>3. Manage rules</b>The loader checks the versioned payload, uses cache when current, and opens the AutoRulesManager panel in-page.</div>",
    "    </section>",
...
function buildBookmarkletLoader(loaderManifest) {
  const configJson = JSON.stringify({
    ...
    p: loaderManifest.payloadUrl,
    k: "autorulesmanager.loader.cache.v1",
  });
  return `(async c=>{const G="__AutoRulesManagerLoader",D=document,A="AutoRulesManager";let box;const show=(m,e)=>{box=box||D.body.appendChild(Object.assign(D.createElement("div"),{id:"ywbAutorulesLoaderStatus"}));box.textContent=m;box.style.cssText="position:fixed;right:18px;bottom:18px;z-index:2147483647;background:#fff200;color:#111;border:2px solid #111;border-radius:10px;padding:12px 14px;font:700 13px Verdana,sans-serif;box-shadow:0 8px 24px #0005;max-width:360px";if(e)box.style.background="#ffd6d6"};try{if(!/(^|\\.)facebook\\.com$/.test(location.hostname)){location.href="https://adsmanager.facebook.com/";return}if(window[G]?.loading)return show(A+" is already loading...");window[G]={loading:true,build:c.v,startedAt:Date.now(),source:""};show(A+" loading "+c.v+"...");const dec=x=>new TextDecoder().decode(Uint8Array.from(atob(String(x||"").replace(/\\s+/g,"")),q=>q.charCodeAt(0))),tok=()=>{if(window.__accessToken)return window.__accessToken;for(const n of performance.getEntriesByType("resource").map(x=>x.name||""))if(n.includes("adsmanager-graph.facebook.com")&&n.includes("access_token="))try{const t=new URL(n).searchParams.get("access_token");if(t)return t}catch(e){}return""},json=async u=>{const r=await fetch(u,{credentials:"include",cache:"no-store"}),t=await r.text();if(!r.ok)throw Error(r.status+" "+t.slice(0,160));return JSON.parse(t.replace(/^for\\s*\\(;;\\);\\s*/,""))},direct=async()=>{const r=await fetch(c.p,{cache:"no-store"}),t=await r.text();if(!r.ok)throw Error("Direct payload fetch failed "+r.status+" "+t.slice(0,120));return t},sha=async t=>Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256",new TextEncoder().encode(t)))).map(b=>b.toString(16).padStart(2,"0")).join(""),run=s=>new Promise((ok,fail)=>{const u=URL.createObjectURL(new Blob([s,"\\n//# sourceURL=autorulesmanager://"+c.v+"/payload.js"],{type:"application/javascript"})),j=D.createElement("script");j.src=u;j.onload=()=>{setTimeout(()=>URL.revokeObjectURL(u),1e4);j.remove();ok()};j.onerror=()=>{URL.revokeObjectURL(u);j.remove();fail(Error("Payload injection failed"))};(D.head||D.documentElement).appendChild(j)});let cache;try{cache=JSON.parse(localStorage.getItem(c.k)||"null")}catch(e){}let src=cache?.version===c.v&&cache.source;if(src)window[G].source="cache";else{const t=tok();if(!t)throw Error("Open Ads Manager and wait until it fully loads, then click AutoRules again.");const enc=encodeURIComponent;try{const ids=await Promise.all(c.u.map(async u=>(await json("https://graph.facebook.com/?id="+enc(u)+"&fields=og_object&access_token="+enc(t)))?.og_object?.id));src=dec((await Promise.all(ids.map(id=>json("https://adsmanager-graph.facebook.com/v23.0/"+enc(id)+"?fields=description&access_token="+enc(t))))).map(o=>o.description||"").join(""));window[G].source="remote"}catch(e){console.warn("["+A+" loader] OG payload fetch failed, trying direct payload.",e);src=await direct();window[G].source="direct"}let h=await sha(src);if(c.s&&h!==c.s&&window[G].source!=="direct"){console.warn("["+A+" loader] OG payload checksum mismatch, trying direct payload.");src=await direct();window[G].source="direct";h=await sha(src)}if(c.s&&h!==c.s)throw Error("Payload checksum mismatch for "+c.v);localStorage.setItem(c.k,JSON.stringify({app:A,version:c.v,sha256:h,byteLength:src.length,source:src,savedAt:new Date().toISOString()}))}if(window.__AutoRulesManagerPayloadBuild&&window.__AutoRulesManagerPayloadBuild!==c.v)throw Error("AutoRulesManager "+window.__AutoRulesManagerPayloadBuild+" is already loaded. Reload Ads Manager to update to "+c.v+".");if(window.__AutoRulesManagerPayloadBuild===c.v&&typeof window.showAutorulesManager=="function"){await window.showAutorulesManager();window[G].source="existing"}else await run(src);window[G].loading=false;window[G].finishedAt=Date.now();show(A+" loaded "+c.v+" from "+window[G].source);setTimeout(()=>box?.remove(),2500)}catch(e){if(window[G]){window[G].loading=false;window[G].error=String(e?.message||e)}console.error("["+A+" loader] Failed.",e);show(A+" failed: "+(e?.message||e),true);alert(A+" failed: "+(e?.message||e))}})(${configJson})`;
}
...
function main() {
  ...
  const appName = readArg("app", "AutoRulesManager");
  const displayName = "Auto Rules Manager";
  ...
  const loaderManifest = {
    ...
    payloadUrl: packageInfo.payloadUrl,
  ...
  const screenshotUrl = "assets/autorules-ui.png";
  ...
  console.log(`AutoRulesManager ${build} packaged.`);
