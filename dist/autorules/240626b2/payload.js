...
class CurrencyConverter {
  ...
  static normalizeField(field) {
    ...
    return field
      .replace(/^(ad|adset|campaign)\./, "")
      .replace(/^(today|lifetime)_/, "");
  }

  static isCurrencyField(field) {
    return this.FIELDS.includes(this.normalizeField(field));
  }
  ...
  static toUSD(rule, conversionRate, fromCurrency) {
    ...
    if (convertedRule.evaluation_spec && convertedRule.evaluation_spec.filters) {
      convertedRule.evaluation_spec.filters.forEach(filter => {
        if (filter.value && !isNaN(filter.value) && this.isCurrencyField(filter.field)) {
...
class AccountManager {
  ...
  normalizeRuleStats(ruleStats = {}) {
    return {
      ruleCount: ruleStats.ruleCount || 0,
      enabledRuleCount: ruleStats.enabledRuleCount || 0,
      disabledRuleCount: ruleStats.disabledRuleCount || 0,
      otherRuleCount: ruleStats.otherRuleCount || 0
    };
  }
  ...
  async loadAll() {
    const api = new FbApi();
    try {
      logger.info("Loading all accounts with rule stats...");
      
      // Get all accounts with autorules status info in a single request using field expansion
      const accounts = await api.getAllPages("me/adaccounts", "fields=id,name,account_status,currency,account_currency_ratio_to_usd,adrules_library.limit(100){id,name,status}");
      
      // Map accounts with rule stats from the data array
      this.accounts = accounts.map(account => {
        const accountId = account.id.replace("act_", "");
        const ruleStats = this.getRuleStatsFromRules(account.adrules_library?.data || []);
  ...
  updateRuleCount(accountId, newCount) {
    ...
    if (account) {
      ...
      if (newCount === 0) {
        account.enabledRuleCount = 0;
        account.disabledRuleCount = 0;
        account.otherRuleCount = 0;
      }
    }
  }
  ...
  setRuleStats(accountId, ruleStats) {
    const account = this.findById(accountId);
    if (account) {
      Object.assign(account, this.normalizeRuleStats(ruleStats));
    }
  }

  addToRuleStats(accountId, ruleStats) {
    const account = this.findById(accountId);
    const normalizedStats = this.normalizeRuleStats(ruleStats);
    if (account) {
      account.ruleCount += normalizedStats.ruleCount;
      account.enabledRuleCount += normalizedStats.enabledRuleCount;
      account.disabledRuleCount += normalizedStats.disabledRuleCount;
      account.otherRuleCount += normalizedStats.otherRuleCount;
    }
  }
}
...
class FbRules {
  ...
  async updateRuleStatus(ruleId, status) {
    ...
    return await this.fb.postRequest(ruleId, body);
  }

  async updateRulesStatusBatch(ruleIds, status) {
    if (!Array.isArray(ruleIds) || ruleIds.length === 0) {
      return [];
    }
    ...
    const batchEntries = ruleIds.map(ruleId => ({
      method: "POST",
      relative_url: ruleId,
      body: `locale=en_US&status=${encodeURIComponent(status)}`
    }));
...
function convertCurrencyInRule(rule, conversionRate, accountCurrency) {
  ...
  if (convertedRule.evaluation_spec && convertedRule.evaluation_spec.filters) {
    convertedRule.evaluation_spec.filters.forEach(filter => {
      ...
      if (filter.value && !isNaN(filter.value) && CurrencyConverter.isCurrencyField(filter.field)) {
...
async function importRulesToAccount(accountId, rules, clearExisting, mainErrorLog = []) {
  ...
  let importedCount = 0;
  let importedEnabledCount = 0;
  let importedDisabledCount = 0;
  let importedOtherCount = 0;
        ...
        for (let i = 0; i < sanitizedRules.length; i++) {
          ...
          try {
            ...
            if (statusCode >= 200 && statusCode < 300 && !(body?.error)) {
              ...
              } else if (rule.status === "ENABLED" || !rule.status) {
    ...
    for (let i = 0; i < accountIds.length; i++) {
      ...
      if (result.success || result.partial) {
        const importedStats = {
          ...
          disabledRuleCount: result.importedDisabledCount,
  ...
  try {
    ...
    for (let i = 0; i < accountIds.length; i++) {
      ...
      try {
        await rulesApi.clearRules(accountId);
        successCount++;
        accountManager.setRuleStats(accountId, {
          ruleCount: 0,
          enabledRuleCount: 0,
          disabledRuleCount: 0,
          otherRuleCount: 0
...
async function setAutorulesStatusForAccounts(accountIds, targetStatus, uiInstance) {
  const rulesApi = new FbRules();
  const desiredStatus = targetStatus === "ENABLED" ? "enabled" : "disabled";
  const actionLabel = targetStatus === "ENABLED" ? "Turn On" : "Turn Off";

  try {
    logger.info(`${actionLabel} started for ${accountIds.length} selected account(s).`);

    let fullySuccessfulAccounts = 0;
    ...
    for (let i = 0; i < accountIds.length; i++) {
      ...
      logger.info(`Processing account ${accountName} (${accountId}) ${i + 1}/${accountIds.length}...`);
      ...
      try {
        ...
        if (rules.length === 0) {
          ...
          logger.info(`Account ${accountName} (${accountId}): no autorules found.`);
          fullySuccessfulAccounts++;
        } else {
          const currentStats = accountManager.getRuleStatsFromRules(rules);
          let enabledRuleCount = currentStats.enabledRuleCount;
          let disabledRuleCount = currentStats.disabledRuleCount;
          let otherRuleCount = currentStats.otherRuleCount;
          const rulesToUpdate = rules.filter(rule => rule.status !== targetStatus);
          const skippedCount = rules.length - rulesToUpdate.length;
          let updatedCount = 0;
          let failedCount = 0;

          if (rulesToUpdate.length > 0) {
            const ruleChunks = chunkArray(rulesToUpdate, Config.BATCH_SIZE);

            for (let chunkIndex = 0; chunkIndex < ruleChunks.length; chunkIndex++) {
              const chunk = ruleChunks[chunkIndex];
              const batchResponse = await rulesApi.updateRulesStatusBatch(
                chunk.map(rule => rule.id),
                targetStatus
              );

              chunk.forEach((rule, responseIndex) => {
                const response = batchResponse?.[responseIndex];
                ...
                try {
                  ...
                  if (statusCode >= 200 && statusCode < 300 && !(body?.error)) {
                    ...
                    if (targetStatus === "ENABLED") {
                      enabledRuleCount++;
                      if (rule.status === "DISABLED") {
                        disabledRuleCount = Math.max(0, disabledRuleCount - 1);
                      } else {
                        otherRuleCount = Math.max(0, otherRuleCount - 1);
                      }
                    } else {
                      disabledRuleCount++;
                      if (rule.status === "ENABLED") {
                        enabledRuleCount = Math.max(0, enabledRuleCount - 1);
                      } else {
                        otherRuleCount = Math.max(0, otherRuleCount - 1);
                      }
                    }
                    ...
                    logger.error(`Account ${accountName} (${accountId}), rule ${rule.name || rule.id}: ${message}`);
                  }
                } catch (responseError) {
                  failedCount++;
                  logger.error(`Account ${accountName} (${accountId}), rule ${rule.name || rule.id}: ${responseError.message || responseError}`);
                }
              });

              if (chunkIndex < ruleChunks.length - 1) {
                logger.info(`Account ${accountName} (${accountId}): batch ${chunkIndex + 1}/${ruleChunks.length} complete. Waiting ${Config.BATCH_DELAY_MS}ms...`);
                await new Promise(resolve => setTimeout(resolve, Config.BATCH_DELAY_MS));
              }
            }
          }
          ...
          totalUpdatedRules += updatedCount;
          totalSkippedRules += skippedCount;

          if (failedCount === 0) {
            fullySuccessfulAccounts++;
            logger.success(`Account ${accountName} (${accountId}): ${updatedCount} rule(s) ${desiredStatus}, ${skippedCount} already ${desiredStatus}.`);
          } else if (updatedCount > 0 || skippedCount > 0) {
            accountsWithErrors++;
            logger.warning(`Account ${accountName} (${accountId}): ${updatedCount} rule(s) ${desiredStatus}, ${skippedCount} already ${desiredStatus}, ${failedCount} failed.`);
          } else {
            failedAccounts++;
            logger.error(`Account ${accountName} (${accountId}): failed to ${targetStatus === "ENABLED" ? "enable" : "disable"} any rules.`);
          }
        }
      } catch (error) {
        failedAccounts++;
        logger.error(`Account ${accountName} (${accountId}): ${error.message || error}`);
      }
    ...
    logger.success(
      `${actionLabel} finished. Accounts: ${fullySuccessfulAccounts} successful, ${accountsWithErrors} with partial errors, ${failedAccounts} failed. Rules ${desiredStatus}: ${totalUpdatedRules}. Already ${desiredStatus}: ${totalSkippedRules}.`
    );
    ...
    logger.error(`Error while running ${actionLabel}: ${error.message || error}`);
  }
}
...
class AutorulesManagerUI {
  ...
  buildAccountOptions(select, selectedIds, accounts = allAccountsData) {
    ...
    accountList.forEach(account => {
      ...
      option.textContent = this.formatAccountOptionLabel(account);
  ...
  formatAccountOptionLabel(account) {
    const total = account.ruleCount || 0;
    const enabled = account.enabledRuleCount || 0;
    const disabled = account.disabledRuleCount || 0;
    const other = account.otherRuleCount || 0;
    let suffix = `${total} rules`;

    if (total > 0) {
      if (other > 0) {
        suffix = `${total} rules: ${enabled} on, ${disabled} off, ${other} other`;
      } else if (enabled === total) {
        suffix = `${total} rules: all on`;
      } else if (disabled === total) {
        suffix = `${total} rules: all off`;
      } else {
        suffix = `${total} rules: ${enabled} on, ${disabled} off`;
      }
    }

    return `${account.id} - ${account.name} [${suffix}]`;
  }
  ...
  applyAccountSearch(select, query, mode) {
    ...
    if (mode === "export") {
      ...
      this.toggleSearchQuery = normalized;
    }
    const selectedIds = mode === "export"
      ? this.selectedExportAccountIds
      : mode === "import"
        ? this.selectedImportAccountIds
        : this.selectedToggleAccountIds;
  ...
  createToggleAccountDropdown() {
    const container = document.createElement("div");
    container.style.width = "100%";
    container.style.margin = "10px 0";

    const label = document.createElement("label");
    label.textContent = "Select accounts for autorules status change:";
    label.style.display = "block";
    label.style.marginBottom = "5px";
    label.style.fontSize = "14px";
    label.style.fontWeight = "bold";
    ...
    const selectAllCheckbox = document.createElement("input");
    selectAllCheckbox.type = "checkbox";
    selectAllCheckbox.id = "ywbSelectAllToggleAccounts";
    selectAllCheckbox.style.marginRight = "5px";

    const selectAllLabel = document.createElement("label");
    selectAllLabel.htmlFor = "ywbSelectAllToggleAccounts";
    selectAllLabel.textContent = "Select All Accounts";
    selectAllLabel.style.fontSize = "12px";
    selectAllLabel.style.fontStyle = "italic";
    ...
    const select = document.createElement("select");
    select.id = "ywbToggleAccountSelect";
    select.multiple = true;
    select.size = Math.min(allAccountsData.length, 8);
    select.style.width = "100%";
    select.style.padding = "5px";
    ...
    this.buildAccountOptions(select, this.selectedToggleAccountIds);

    const searchInput = this.createSearchInput(
      "ywbToggleAccountSearch",
      "Search by name or ID",
      value => this.applyAccountSearch(select, value, "toggle")
    );

    const updateSelection = () => {
      this.selectedToggleAccountIds = Array.from(select.selectedOptions).map(opt => opt.value);
    };
  ...
  refreshDropdowns() {
    ...
    const toggleSelect = document.getElementById("ywbToggleAccountSelect");
    ...
    if (toggleSelect) {
      this.buildAccountOptions(toggleSelect, this.selectedToggleAccountIds);
      this.applyAccountSearch(toggleSelect, this.toggleSearchQuery, "toggle");
    }
  }
  ...
  createTabs() {
    ...
    toggleTab.id = "ywbToggleTab";
    toggleTab.textContent = "On/Off";
    toggleTab.style.flex = "1";
    toggleTab.style.padding = "10px";
    toggleTab.style.border = "none";
    toggleTab.style.background = "none";
    ...
    const activateTab = tabName => {
      const tabMap = {
        export: {
          button: exportTab,
          contentId: "ywbExportTabContent"
        },
        import: {
          button: importTab,
          contentId: "ywbImportTabContent"
        },
        toggle: {
          button: toggleTab,
          contentId: "ywbToggleTabContent"
        }
      };
      ...
      activeTab.button.style.borderBottom = "3px solid #333";
    ...
    tabContainer.appendChild(toggleTab);
  ...
  show() {
    ...
    // Toggle Tab Content
    const toggleTabContent = document.createElement("div");
    toggleTabContent.id = "ywbToggleTabContent";
    toggleTabContent.style.width = "100%";
    toggleTabContent.style.display = "none";

    const toggleDropdown = this.createToggleAccountDropdown();

    const toggleButtonsRow = document.createElement("div");
    toggleButtonsRow.style.display = "flex";
    toggleButtonsRow.style.gap = "10px";
    toggleButtonsRow.style.width = "100%";

    const turnOnButton = this.createButton("turn-on-btn", "Turn On", async () => {
      if (!this.selectedToggleAccountIds || this.selectedToggleAccountIds.length === 0) {
        alert("Please select at least one account.");
        return;
      }

      await setAutorulesStatusForAccounts(this.selectedToggleAccountIds, "ENABLED", this);
    });
    turnOnButton.style.margin = "10px 0";
    turnOnButton.style.flex = "1";
    turnOnButton.style.width = "auto";

    const turnOffButton = this.createButton("turn-off-btn", "Turn Off", async () => {
      if (!this.selectedToggleAccountIds || this.selectedToggleAccountIds.length === 0) {
        alert("Please select at least one account.");
        return;
      }

      await setAutorulesStatusForAccounts(this.selectedToggleAccountIds, "DISABLED", this);
    });
    turnOffButton.style.margin = "10px 0";
    turnOffButton.style.flex = "1";
    turnOffButton.style.width = "auto";
    turnOffButton.style.backgroundColor = "#d9534f";

    toggleButtonsRow.appendChild(turnOnButton);
    toggleButtonsRow.appendChild(turnOffButton);

    toggleTabContent.appendChild(toggleDropdown);
    toggleTabContent.appendChild(toggleButtonsRow);

    // Add tab contents to div
    div.appendChild(exportTabContent);
    div.appendChild(importTabContent);
    div.appendChild(toggleTabContent);
...
async function showAutorulesManager() {
  try {
    ...
    logger.setUI(ui);
    ui.show();
    logger.success(`Loaded ${accountManager.getAll().length} accounts.`);
  } catch (error) {
...
${setAutorulesStatusForAccounts.toString()}
